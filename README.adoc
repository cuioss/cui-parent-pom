...
* Java version is set explicitly: <maven.compiler.source>17</maven.compiler.source> and <maven.compiler.target>17</maven.compiler.target> in the root pom.xml, inherited by all modules.
...
* Regular reviews are performed to check for unpinned dependencies, plugins, or new sources of non-determinism.
