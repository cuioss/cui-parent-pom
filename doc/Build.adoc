...
To update the Maven Wrapper scripts (mvnw, mvnw.cmd, .mvn/wrapper), run the following command, pinning the desired Maven version:
...
This uses the official `org.apache.maven.plugins:maven-wrapper-plugin`. The wrapper is configured with `distributionType=only-script`.
